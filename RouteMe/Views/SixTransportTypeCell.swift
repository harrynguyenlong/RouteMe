...
class SixTransportTypeCell: MainBaseCell {
    ...
    var intinery: ItineryDetail? {
        didSet {
            ...
            sixthBaseView.baseImage.image = getIconWithCorrespondingMode(mode: itinery.legs[5]?.mode ?? .walk)
            sixthBaseView.descriptionLbl.text = getDescriptionLabel(mode: itinery.legs[5]?.mode ?? .walk, itinery: itinery, index: 5)
        }
    }
    ...
    lazy var fifthArrow: UIImageView = {
        let arrow = UIImageView()
        arrow.image = #imageLiteral(resourceName: "right-arrow")
        arrow.clipsToBounds = true
        arrow.contentMode = .scaleAspectFill
    ...
    lazy var sixthBaseView: BaseView = {
        let view = BaseView()
        view.backgroundColor = .clear
        return view
    }()
    ...
    override func setupView() {
        ...
        addSubview(fifthArrow)
        
        fifthArrow.anchor(top: topAnchor, left: fifthBaseView.rightAnchor, bottom: nil, right: nil, paddingTop: 30, paddingLeft: 0, paddingBottom: 0, paddingRight: 0, width: 10, height: 10)
        
        addSubview(sixthBaseView)
        
        sixthBaseView.anchor(top: topAnchor, left: fifthArrow.rightAnchor, bottom: travelTimeLbl.topAnchor, right: nil, paddingTop: 5, paddingLeft: 0, paddingBottom: 0, paddingRight: 0, width: 40, height: 0)
    }
